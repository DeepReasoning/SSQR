...
class VQGCN_MLP(nn.Module):
    def __init__(self, num_ent, num_rel, gcn_layers, tf_layers, init_dim, gcn_dim, edge_type, edge_norm,
        ...
        self.encoder = nn.Linear(gcn_dim, gcn_dim*seq_len)
        self.decoder = nn.Linear(gcn_dim*seq_len, gcn_dim)
        self.codebook = Codebook(num_codes=num_code, latent_dim=gcn_dim)

        # x loss
        self.line1 = nn.Linear(gcn_dim*seq_len, 2048)
        self.line2 = nn.Linear(2048, 3072)
        ...
        self.loss = nn.BCELoss()
        self.x_loss = nn.MSELoss()
        # self.x_loss = nn.L1Loss()
    ...
    def forward(self, g, subj, rel, ent_text_emd=None, stage=1):
        ...
        # all_ent = self.kggcn.init_embed
        # sub_emb = all_ent[subj]
        # rel_emb = self.kggcn.init_rel[rel]


        batch_size = sub_emb.size(0)

        sub_trans = self.encoder(sub_emb).view(batch_size, self.seq_len, -1)  # B L D
        ...
        if stage != 1:  # 2 3
            sub_trans = sub_trans_q  # 增加原始CLS
        sub_trans = sub_trans.view(batch_size, -1)

        # 特征损失
        if ent_text_emd == None:
            x_loss = 0
        else:
            rec_x = self.line1(sub_trans)
            rec_x = torch.relu(rec_x)
            rec_x = self.drop(rec_x)
            rec_x = self.line2(rec_x)
            ori_x = ent_text_emd[subj]
            x_loss = self.x_loss(rec_x, ori_x)

        sub_final = self.decoder(sub_trans)
    ...
    def cal_allent_codes(self, g):
        ...
        batch_size = all_ent.size(0)
        sub_trans = self.encoder(all_ent).view(batch_size, self.seq_len, -1)  # B L D
    ...
    def cal_allent_codes_emds(self, g):
        ...
        batch_size = all_ent.size(0)
        sub_trans = self.encoder(all_ent).view(batch_size, self.seq_len, -1)  # B L D
        sub_trans = self.codebook.cal_code_emds(sub_trans)  # 不取CLS
        return sub_trans
...
class Codebook(nn.Module):  # embedding
    ...
    def cal_code_emds(self, z):  # 生成code B L D
        z_flattened = z.reshape(-1, self.latent_dim)  # M D
        d = torch.sum(z_flattened ** 2, dim=1, keepdim=True) + \
            torch.sum(self.embedding.weight ** 2, dim=1) - \
            2 * (torch.matmul(z_flattened, self.embedding.weight.t()))  # M,1 N -> MN - MN -> MN
